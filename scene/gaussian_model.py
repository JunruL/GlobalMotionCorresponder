...
class GaussianModel:
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation, isotropic=False):
            L = build_scaling_rotation(scaling_modifier * scaling, rotation, isotropic)
    ...
    def __init__(self, sh_degree : int, isotropic : bool = False):
    ...
    def get_scaling(self):
        if self.isotropic:
            return self.scaling_activation(self._scaling[:, 0].unsqueeze(1).repeat(1, 3))
        else:
    ...
    def get_semantic_feature(self):
        return self._semantic_feature.squeeze(1)
    
    @property
    def get_rgb(self):
        c0 = 0.28209479177387814
        sh2rgb = self._features_dc[:, 0, :] * c0
        return torch.clamp_min(sh2rgb + 0.5, 0.0)
    
    def add_xyz(self, value, fg_ids=None):
        if fg_ids is not None:
            #  make the value have the same size with xyz by filling 0 in non fg_ids positions
            _value = torch.zeros_like(self._xyz)
            _value[fg_ids] = value
            self._xyz = self._xyz + _value
        else:
            self._xyz = self._xyz + value
        
    def set_xyz(self, value, fg_ids=None):
        if fg_ids is not None:
            #  make the value have the same size with xyz by filling 0 in non fg_ids positions
            _value = torch.zeros_like(self._xyz)
            _value[fg_ids] = value
            fg_ids_tensor = torch.tensor(fg_ids, device=self._xyz.device)
            non_fg_ids = torch.arange(self._xyz.size(0), device=self._xyz.device)[~torch.isin(torch.arange(self._xyz.size(0), device=self._xyz.device), fg_ids_tensor)]
            _value[non_fg_ids] = self._xyz[non_fg_ids]
            self._xyz = _value
        else:
            self._xyz = value
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, semantic_feature_size : int):
        ...
        # From gsplat:
        # Initialize the GS size to be the average dist of the nearest neighbor
        dist = torch.clamp_min(torch.from_numpy(knn(np.asarray(pcd.points), 2)[:, 1]).float().cuda(), 0.0000001)
        scales = torch.log(dist).unsqueeze(-1).repeat(1, 3)  # [N, 3]
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, width, height):
        grad = viewspace_point_tensor.grad[0, update_filter,:2]
